...
- 自动识别上班时间（可配置时间段内首次亮屏/登录/开盖事件，默认 8:00-12:00）
- 自动识别下班时间（18:00 后最后一次屏幕关闭/合盖睡眠事件）
- 菜单栏实时显示：预计下班时间 / 剩余倒计时（<= 30 分钟显示） / 已下班
- 菜单栏日历：按月查看农历、节假日与调休（休/班标记），点击日期查看该日考勤
- 考勤历史持久化：上下班记录全量保存在本地，不受系统日志轮转影响
...
- 农历日期（基于 `6tail/lunar-go`）
- 节假日与调休标记：休息日显示「休」，调休上班日显示「班」
- 节日名称与今日标记（●）

点击任意日期可查看该日考勤详情。节假日数据默认从 iCloud 中国大陆日历接口拉取并缓存 24 小时，可通过菜单「设置节假日接口...」自定义 ICS 地址。
...
| 节假日接口 | iCloud 中国大陆 ICS | 可通过菜单栏「设置节假日接口...」修改 |

## 数据存储

- 配置：`~/.worktime/config.json`
- 考勤历史：`~/.worktime/events.json`（全量持久化，与 pmset 日志合并读取）
- 节假日缓存：`~/.worktime/holiday.ics`（24 小时刷新）
...
- [6tail/lunar-go](https://github.com/6tail/lunar-go) — 农历与节假日计算
